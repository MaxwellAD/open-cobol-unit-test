      ...
      *> OPTIONAL EXTRA CONTEXT FOR THE EXPECT-DUT-* HELPERS. SET IT ON
      *> THE LINE BEFORE THE PERFORM AND IT IS APPENDED TO THE FAILURE
      *> MESSAGE. THE HELPERS CLEAR IT EITHER WAY, SO IT CANNOT LEAK
      *> INTO A LATER ASSERTION
       01 EXPECT-DETAIL               PIC X(80) VALUE SPACES.
      *> THE DETAIL WITH ITS SEPARATOR ATTACHED, OR SPACES WHEN NO
      *> DETAIL WAS SET - SO ONE STRING SERVES BOTH CALL STYLES
       01 EXPECT-SUFFIX               PIC X(83) VALUE SPACES.
      ...
      *> BLANK - KEEPS EACH HELPER TO A SINGLE STRING STATEMENT
       EXPECT-BUILD-SUFFIX SECTION.
           MOVE SPACES TO EXPECT-SUFFIX
           IF EXPECT-DETAIL NOT = SPACES
              STRING ' - ' FUNCTION TRIM(EXPECT-DETAIL)
                 DELIMITED BY SIZE INTO EXPECT-SUFFIX
              END-STRING
           END-IF
           .

       EXPECT-DUT-PASSED SECTION.
           PERFORM EXPECT-BUILD-SUFFIX
           IF NOT DUT-TEST-PASS
              STRING 'EXPECTED THE DUT CASE TO PASS BUT STATUS WAS '
                     DUT-TEST-STATUS EXPECT-SUFFIX
                 DELIMITED BY SIZE INTO CUT-DISPLAY-FAIL-MSG
              END-STRING
              PERFORM CUT-FAIL
           END-IF
           MOVE SPACES TO EXPECT-DETAIL
           .
