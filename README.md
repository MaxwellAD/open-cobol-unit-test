...
An open source automated COBOL Unit Test library 
- (under construction, please look at the `prototype` branch for bleeding edge source code and extra documentation, or continue reading here for an overview)

Designed to be xUnit style
...
This breadcrumb also takes a snapshot of various working storage fields, which you register in the test program
```COBOL
...
Capturing this data is fine, but it needs to be query-able in a reasonably easy way which is where the `CUTSTOR` and `CUTPROC` come in
...
Everything inside CUTSTOR and CUTPROC is prefix with "CUT-" (COBOL Unit Test) e.g `01  CUT-DATA.`. To avoid obvious naming conflicts, the framework is testing an imaginary program with "CUT-" replaced with "DUT-", for "Dummy Unit Test" e.g `01  DUT-DATA.` which enables an effective, albeit confusing unit test suite

Seen in Examples/Example01/

The latest stable version of the library (as of writing) has a code coverage of 98.2% across 36 meaningful unit tests which can be executed in milliseconds

New features can be implemented into DUT and have their behaviors observed before being added to CUT. Making for a much easier development process

Having the inner workings of each section documented by a unit test program makes expanding the capabilities and understanding the logic after some time away much easier


# The Output
When run against Examples/Example01/test-pgm.cbl
```
TEST CASE - TEST-INIT-CLEARS-TRACE
[PASS]

...


TEST CASE - TEST-END-TEST-SUITE-NOERR
[PASS]

TEST CASE - TEST-DUT-SKIP
[PASS]

TEST CASE - TEST-REGISTER-FIELD-REGISTERS
[PASS]
...
This project is currently under construction and has no "Version 1" ready, please see the `prototype` branch for the latest working version which contains more documentation

As said earlier, Example01 is the most comprehensive example at the time of writing, purely because it's also the testing mechanism for this framework

As this project becomes more mature, other examples will be created to highlight the strengths and improvements needed for this framework
